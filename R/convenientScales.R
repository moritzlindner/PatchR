...
setMethod("convenientScales",
          ...
          function(object){
            for (i in 1:length(getTraceNames(object))){
              decimals<-seq(0,21,3)
              leading0<-median(attr(regexpr("(?<=\\.)0+", object@Data[[getTraceNames(object)[i]]], perl = TRUE), "match.length")) # gives average no of leading zeros
              leading0<-min(decimals[decimals>leading0])
              object@Units[[i]]<-gsub("[^a-zA-Z]", "", sitools::f2si(10^-(leading0),object@Units[[i]]))
              object@Data[[getTraceNames(object)[i]]]<-object@Data[[getTraceNames(object)[i]]]*10^(leading0)
            }
            object
          }
)
